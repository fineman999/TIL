...
public class RegisterTest {
  ...
  @DisplayName("임시 저장")
  @Test
  void register() throws PopbillException {
    /**
     * 작성된 세금계산서 데이터를 팝빌에 저장합니다. - "임시저장" 상태의 세금계산서는 발행(Issue) 함수를 호출하여 "발행완료" 처리한 경우에만 국세청으로 전송됩니다.
     * - 정발행 시 임시저장(Register)과 발행(Issue)을 한번의 호출로 처리하는 즉시발행(RegistIssue API) 프로세스 연동을 권장합니다. - 역발행 시
     * 임시저장(Register)과 역발행요청(Request)을 한번의 호출로 처리하는 즉시요청(RegistRequest API) 프로세스 연동을 권장합니다. - 세금계산서
     * 파일첨부 기능을 구현하는 경우, 임시저장(Register API) -> 파일첨부(AttachFile API) -> 발행(Issue API) 함수를 차례로 호출합니다.
     * - 역발행 세금계산서를 저장하는 경우, 객체 'Taxinvoice'의 변수 'chargeDirection' 값을 통해 과금 주체를 지정할 수 있습니다. └ 정과금 :
     * 공급자 과금 , 역과금 : 공급받는자 과금 - 임시저장된 세금계산서는 팝빌 사이트 '임시문서함'에서 확인 가능합니다. -
    ...
    taxinvoice.setInvoicerMgtKey("20240728-BOOT003");
    ...
    Response response = taxinvoiceService.register(popbillConfig.getCorpNum(), taxinvoice);
