...
public class ValidatorConfiguration {
    ...
    @Bean
    public Job batchJob(JobRepository jobRepository, Step testStep1, Step testStep2, Step testStep3) {
...
//                .validator(new CustomJobParametersValidator())
    ...
    @Bean
    public Step testStep3(JobRepository jobRepository, Tasklet testTask3, PlatformTransactionManager transactionManager) {
        return new StepBuilder("testStep3", jobRepository)
                .tasklet(testTask3, transactionManager)
                .build();
    }

    @Bean
    public Tasklet testTask3() {
        return (contribution, chunkContext) -> {
            System.out.println("--------------------");
