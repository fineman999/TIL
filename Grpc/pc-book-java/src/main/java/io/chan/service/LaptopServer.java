...
public class LaptopServer {
  ...
  public LaptopServer(
      ...
      final ServerCredentials serverCredentials) throws IOException {
    this.port = port;
    this.server =
  ...
  public static ServerCredentials loadTLSCredentials() throws IOException {
    final File serverCertFile = new File("cert/server-cert.pem");
    final File serverKeyFile = new File("cert/server-key.pem");
    final File caFile = new File("cert/ca-cert.pem");
  ...
  /**
   * 메소드는 서버가 완전히 종료될 때까지 최대 30초 동안 대기합니다. 이 시간이 지나면 메소드는 반환되며, 이후에 서버가 종료되더라도 추가적인 액션은 취하지 않습니다. 이
   * 메소드는 서버의 모든 리소스가 제대로 해제되도록 보장하는 역할을 합니다.
   */
  ...
  /**
   * 이 코드에서는 main 메소드에서 blockUntilShutdown() 메소드를 호출하므로, main 스레드가 블록됩니다. 이는 서버가 계속 실행되도록 하기 위한
   * 것입니다. 서버가 종료되면 main 스레드도 종료되고, 따라서 프로그램도 종료됩니다. 따라서 이 메소드는 서버가 계속 실행되도록 main 스레드를 블록하는 역할을 합니다
   */
  ...
  public static void main(String[] args) throws InterruptedException, IOException {
    ...
    final LaptopServer laptopServer =
        new LaptopServer(port, laptopStore, imageStore, ratingStore, loadTLSCredentials());
    laptopServer.start();
