...
public class LaptopClient {
  ...
  public void rateLaptop(String[] laptopIds, double[] scores) throws InterruptedException {
    ...
    StreamObserver<RateLaptopRequest> requestStreamObserver =
        asyncStub
            .withDeadlineAfter(5, TimeUnit.SECONDS)
    ...
    try {
      ...
      for (int i = 0; i < n; i++) {
        ...
        logger.info(
            "Sent rate laptop request for laptop ID: "
                + laptopIds[i]
                + " with score: "
                + scores[i]);
      }
  ...
  private static void testRateLaptop(Generator generator, LaptopClient client)
    ...
    while (true) {
      ...
      for (int i = 0; i < n; i++) {
        logger.info("Input score for laptop ID: " + laptopIds[i]);
        scores[i] = generator.newLaptopStore();
      }
  ...
  private static void testUploadImg(Generator generator, LaptopClient client)
      throws InterruptedException {
