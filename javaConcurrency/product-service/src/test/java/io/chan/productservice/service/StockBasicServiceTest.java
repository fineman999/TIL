...
class StockBasicServiceTest extends AcceptanceTest {
  ...
  @Autowired private StockThreadSafeService stockThreadSafeService;
  ...
  void decrease100() throws InterruptedException {
    ...
    @Test
    @DisplayName("동시에 100개의 재고를 감소시킨다. - synchronized")
    void synchronizedDecrease100WithoutTransactional() throws InterruptedException {
        int threadCount = 100;
        // 32개의 쓰레드 풀을 생성한다.
        final ExecutorService executorService = Executors.newFixedThreadPool(32);
        // 100개의 쓰레드가 모두 종료될 때까지 대기한다. - 다른 쓰레드가 종료될 때까지 대기한다.
    ...
    @Test
    @DisplayName("동시에 100개의 재고를 감소시킨다. - 외부에서 synchronized 처리")
    void synchronizedDecrease100WithTransactional() throws InterruptedException {
        int threadCount = 100;
        // 32개의 쓰레드 풀을 생성한다.
        final ExecutorService executorService = Executors.newFixedThreadPool(32);
        // 100개의 쓰레드가 모두 종료될 때까지 대기한다. - 다른 쓰레드가 종료될 때까지 대기한다.
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.execute(
                    () -> {
                        try {
                            stockThreadSafeService.decrease(1L, 1L);
                        } finally {
                            countDownLatch.countDown();
                        }
                    });
