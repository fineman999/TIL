...
contract Auction {
    ...
    event sendRefundAmount(address indexed bidder, uint money);
    event sendUpdateAmount(address indexed bidder, uint money);
    /// 입찰 처리 함수
    function bid() public payable {
        ...
        if(!currentHighestBidder.send(refundAmount)) {
            emit sendRefundAmount(currentHighestBidder, refundAmount);
            throw;
        }
        emit sendUpdateAmount(highestBidder, highestBid);
    }
}
