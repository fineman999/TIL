package io.chan.resilience4jpractice.d_circuitbreaker_event;
    ...
    public RegistryEventConsumer<CircuitBreaker> myRegistryEventConsumer() {
        ...
        return new RegistryEventConsumer<CircuitBreaker>() {
            ...
            public void onEntryAddedEvent(EntryAddedEvent<CircuitBreaker> entryAddedEvent) {
                ...
                eventPublisher.onCallNotPermitted(event -> log.info("onCallNotPermitted {}", event)); // CircuitBreaker가 열려(OPEN)있을 때 호출 시도
                eventPublisher.onError(event -> log.info("onError {}", event));
                eventPublisher.onIgnoredError(event -> log.info("onIgnoredError {}", event));

                eventPublisher.onStateTransition(event -> log.info("onStateTransition {}", event)); // CircuitBreaker 상태 전이(변경)
